...
class Helper {
	...
	public static function schedule_import_campaigns( $params ) {
		...
		switch ( $frequency ) {
			...
			case 'weekly':
				...
				break;
			default:
				return new \WP_Error( 'invalid_frequency', __( 'Invalid frequency.', 'campaign-monitor-to-wp' ), array( 'status' => 400 ) );
		}
		...
		if ( 0 === $schedule_id ) {
			return new \WP_Error( 'failed_to_schedule_import', __( 'Failed to schedule import.', 'campaign-monitor-to-wp' ), array( 'status' => 400 ) );
		} else {
			return $schedule_id;
		}
	}
	...
	public static function include_action_scheduler() {
		...
		if ( ! is_plugin_active( 'action-scheduler/action-scheduler.php' ) ) {
			require_once INTEGRATION_TOOLKIT_FOR_BEEHIIV_PATH . 'vendor/woocommerce/action-scheduler/action-scheduler.php';
		}
	}
	...
	public static function filter_campaign_content( $campaign_content ) {
		...
		$headerDiv = $dom->getElementById('web-header');
		...
		// Apply any filters
		return apply_filters( 'itfb_filter_campaign_content', $modifiedHtml );
	}
